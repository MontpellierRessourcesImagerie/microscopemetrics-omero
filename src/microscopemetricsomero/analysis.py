...
def generate_analysis_annotation(start_time, end_time, analysis_config) -> dict:
    return {"analysis_class": analysis_config["analysis_class"],
...
def _analyze_image(conn: BlitzGateway, image: ImageWrapper, analysis_config: dict) -> None:
    ...
    analysis.set_data(analysis_config["data"]["name"], omero_tools.get_image_intensities(image))
    for par, val in analysis_config["parameters"].items():
        analysis.set_metadata(par, val)

    module_logger.info(f"Running analysis {analysis_config['analysis_class']} on image {image.getId()}")
    ...
    omero_tools.create_key_value(conn=conn,
...
def _dump_image_analysis(conn: BlitzGateway,
    ...
    for output_property in analysis.output.get_properties():
        ...
        module_logger.info(f"Dumping {output_property.type} {output_property.name} from analysis {analysis.name}")
        property_types_to_dump[output_property.type](conn, output_property, image)
        module_logger.info(f"Dumping {output_property.type} {output_property.name} from analysis {analysis.name} completed")


def _dump_output_image(conn, output_image, image):
    new_image = omero_tools.create_image(conn=conn,
...
def _dump_output_tag(conn, output_tag, object):
    omero_tools.create_tag(conn=conn,
...
def _dump_output_table(conn, output_table, omero_object):
    omero_tools.create_table(conn=conn,
...
def _dump_comment(conn, output_comment, omero_object):
    omero_tools.create_comment(conn=conn,
...
def analyze_dataset(conn, script_params, dataset, config):
    ...
    for analysis_name, analysis_config in config["analyses_config"]["ANALYSES"].items():
        if analysis_config["do_analysis"]:
            ...
            images = omero_tools.get_tagged_images_in_dataset(dataset, analysis_config["data"]["tag_id"])
